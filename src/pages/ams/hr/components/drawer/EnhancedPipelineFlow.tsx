    ...
    '&::-webkit-scrollbar-thumb': {
      ...
      {stages.map((stage, index) => <React.Fragment key={stage.id}>
        ...
        scrollSnapAlign: 'start'
            ...
            <EnhancedStageCard stage={stage} index={index} onRemove={() => onRemoveStage(stage.id)} onReorder={onReorderStages} onConfigure={() => onConfigureStage(stage)} onEdit={onEditStage ? () => onEditStage(stage) : undefined} onDuplicate={onDuplicateStage ? () => onDuplicateStage(stage) : undefined} onView={onViewStage ? () => onViewStage(stage) : undefined} />
          </Box>
          {index < stages.length - 1 && <Box sx={{
        display: 'flex',
        alignItems: 'center',
        height: '240px',
        minWidth: '48px',
