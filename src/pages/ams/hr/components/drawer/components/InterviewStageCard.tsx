...
import { Avatar, Box, Card, Chip, Divider, IconButton, Typography } from '@mui/material';
import { Check as CheckIcon, MoreHoriz as MoreHorizIcon, ArrowUpward as ArrowUpwardIcon, LocalOffer as LocalOfferIcon, CalendarToday as CalendarIcon, Schedule as ScheduleIcon } from '@mui/icons-material';
...
const InterviewStageCard: React.FC<InterviewStageCardProps> = ({
  ...
  const statusText = stage.status === 'configured' ? 'Configured' : stage.status === 'partially-configured' ? 'Partial' : 'Not Set';
  ...
  return <Card ref={ref} data-handler-id={handlerId} sx={{
    width: '280px',
    height: '180px',
    borderRadius: '16px',
    position: 'relative',
    ...
    '&:hover': {
      ...
      gap: '6px'
        ...
        <Chip label={stage.config?.interviewFormat || 'No format'} sx={{
      ...
      gap: '12px'
        ...
        {/* TA Assignment */}
        {stage.taAssigned ? <Avatar src={stage.taAssigned.avatar} sx={{
        width: '18px',
        height: '18px',
        border: 'none',
        fontSize: '9px',
      ...
      {/* Menu */}
      <StageCardMenu anchorEl={anchorEl} onClose={handleMenuClose} onConfigure={onConfigure} onEdit={onEdit} onDuplicate={onDuplicate} onView={onView} onDelete={onRemove} />
    </Card>;
};
