...
interface EnhancedChecklistsStepProps {
  form: UseFormReturn<RoleCreationFormValues>;
}

const EnhancedChecklistsStep: React.FC<EnhancedChecklistsStepProps> = ({ form }) => {
  const [activeTab, setActiveTab] = useState('general');
  const [customItem, setCustomItem] = useState('');
  
  const { checklistsLibrary } = useRoleLibraries();
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="role" className="space-y-4">
            <Card>
              ...
              <CardContent>
                {getChecklistsByType('role_based').length > 0 ? (
...
export default EnhancedChecklistsStep;
