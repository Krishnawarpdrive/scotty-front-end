...
serve(async (req) => {
  ...
  try {
    ...
    switch (action) {
      case 'get_ta_profiles':
        return await getTAProfiles(supabaseClient)
...
async function getTAProfiles(supabase: any) {
...
async function getTAWorkloads(supabase: any) {
  // Calculate workloads for all TAs by joining with assignments
  const { data, error } = await supabase
    .rpc('calculate_ta_workloads');
